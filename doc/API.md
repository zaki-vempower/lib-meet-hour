...
You can use Meet Hour API to create Meet Hour video conferences with custom GUI.

Installation
==========

To embed Meet Hour API in your application you need to add Meet Hour API library

```html
<script src="https://meethour.io/libs/lib-meet-hour.min.js"></script>
```

Now you can access Meet Hour API trough the `JitsiMeetJS` global object.

Components
=========

Meet Hour API has the following components:
...
*  `JitsiMeetJS.init(options)` - this method initialized Meet Hour API.
...
1. `JitsiConnection(appID, token, options)` - constructor. Creates the conference object.
    ...
    - `appID` - identification for the provider of Meet Hour video conferencing services. **NOTE: not implemented yet. You can safely pass `null`**
    - `token` - secret generated by the provider of Meet Hour video conferencing services. The token will be send to the provider from the Meet Hour server deployment for authorization of the current client.
...
1. The first thing you must do in order to use Meet Hour API is to initialize `JitsiMeetJS` object:
